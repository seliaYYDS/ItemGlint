...
public class ItemTagSelectorBrowserScreen extends Screen {
    ...
    private final List<GuiEventListener> resultWidgets = new ArrayList<>();
    ...
    protected void init() {
        ...
        y += 28;
        final int searchY = y;
        final int listTop = searchY + 28;

        this.searchBox = new EditBox(this.font, left, searchY, panelWidth, 20, Component.translatable("screen.itemglint.selector_browser.search"));
        ...
        this.searchBox.setResponder(value -> {
            ...
            this.page = 0;
            refreshFilter();
            rebuildResultWidgets(left, listTop, panelWidth, right, footerY);
        });
    ...
    private void rebuildResultWidgets(int left, int listTop, int panelWidth, int right, int footerY) {
        clearResultWidgets();

        int pageSize = getPageSize(listTop, footerY - 8);
        ...
        for (int index = start; index < end; index++) {
            ...
            addResultWidget(new EntryWidget(left, listTop + (index - start) * 24, panelWidth, entry, this::selectEntry));
        }

        if (this.page > 0) {
            addResultWidget(Button.builder(Component.translatable("screen.itemglint.selector_browser.prev"), button -> {
                        this.page--;
                        rebuildResultWidgets(left, listTop, panelWidth, right, footerY);
                    }).bounds(left, footerY, 80, 20).build());
        }
        if ((this.page + 1) * pageSize < this.filteredEntries.size()) {
            addResultWidget(Button.builder(Component.translatable("screen.itemglint.selector_browser.next"), button -> {
                        this.page++;
                        rebuildResultWidgets(left, listTop, panelWidth, right, footerY);
    ...
    private <T extends GuiEventListener & net.minecraft.client.gui.components.Renderable & net.minecraft.client.gui.narration.NarratableEntry> T addResultWidget(T widget) {
        this.resultWidgets.add(widget);
        return this.addRenderableWidget(widget);
    }

    private void clearResultWidgets() {
        for (GuiEventListener widget : this.resultWidgets) {
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.drawCenteredString(this.font, Component.translatable("screen.itemglint.selector_browser.title", this.currentMode.label()), this.width / 2, 12, opaque(0xFFFFFF));
        graphics.drawString(this.font,
                Component.translatable("screen.itemglint.selector_browser.count", this.filteredEntries.size(), this.currentMode.label()),
                getPanelLeft(), this.height - 42, opaque(0xAAB3C3), false);
    }
    ...
    private static List<Entry> buildEntries(BrowserMode mode) {
        ...
        switch (mode) {
            ...
            case TAG -> BuiltInRegistries.ITEM.getTags()
    ...
    private static Entry buildItemEntry(Identifier id) {
    ...
    private static final class EntryWidget extends AbstractWidget {
        ...
        protected void renderWidget(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
            ...
            graphics.drawString(font, font.plainSubstrByWidth(this.entry.title, Math.max(10, textRight - textLeft)), textLeft, getY() + 2, opaque(0xE0E6EE), false);
            if (!this.entry.subtitle.isEmpty()) {
                graphics.drawString(font, font.plainSubstrByWidth(this.entry.subtitle, Math.max(10, textRight - textLeft)), textLeft, getY() + 11, opaque(0x8FA1BA), false);
            }
            graphics.drawString(font, Component.translatable("screen.itemglint.selector_browser.add"), getX() + this.width - 34, getY() + 6, opaque(0xA7E7FF), false);
        }
