...
public class ShaderQuickConfigScreen extends Screen {
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.drawCenteredString(this.font, this.title, this.width / 2, 16, opaque(0xFFFFFF));
        graphics.drawString(this.font, Component.translatable(this.currentPage.titleKey), getMainContentLeft(), 36, opaque(0xA7E7FF), false);
        graphics.drawString(this.font, Component.translatable("screen.itemglint.quick_config.pages"), getSidebarLeft() + getSidebarInset(), 52, opaque(0xAAB3C3), false);
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        ...
        if (dispatchMouseClickTopmost(event, doubleClick)) {
            return true;
        }
    ...
    public boolean keyPressed(KeyEvent event) {
        if (this.getFocused() instanceof EditBox editBox && editBox.keyPressed(event)) {
            return true;
        }
    ...
    public boolean charTyped(CharacterEvent event) {
        if (this.getFocused() instanceof EditBox editBox && editBox.charTyped(event)) {
            return true;
        }
    ...
    private void renderStoredText(GuiGraphics graphics) {
        for (LabelLine label : this.labels) {
            ...
            graphics.drawString(this.font, plain, drawX, label.y, opaque(label.color), false);
        }
    ...
    private void renderHoveredTooltip(GuiGraphics graphics, int mouseX, int mouseY) {
        for (int i = this.rowTooltips.size() - 1; i >= 0; i--) {
            ...
            if (area.contains(mouseX, mouseY)) {
                graphics.setTooltipForNextFrame(this.font, this.font.split(area.tooltip(), 220), mouseX, mouseY);
                return;
            }
        }
    }
    ...
    private void renderSidebarSummary(GuiGraphics graphics) {
        ...
        graphics.drawString(this.font, Component.translatable("screen.itemglint.quick_config.summary.title"), x, y, opaque(0xA7E7FF), false);
        y += 16;
        graphics.drawString(this.font, Component.translatable("screen.itemglint.quick_config.summary.status", onOffLabel(this.featureEnabled)), x, y, opaque(0xE0E6EE), false);
        y += 12;
        graphics.drawString(this.font, Component.translatable("screen.itemglint.quick_config.summary.hands", describeHands()), x, y, opaque(0xE0E6EE), false);
        y += 12;
        graphics.drawString(this.font, Component.translatable("screen.itemglint.quick_config.summary.mode", colorModeLabel(this.colorMode)), x, y, opaque(0xE0E6EE), false);
        y += 12;
        graphics.drawString(this.font, Component.translatable("screen.itemglint.quick_config.summary.bloom", onOffLabel(this.bloom)), x, y, opaque(0xE0E6EE), false);
        y += 12;
        graphics.drawString(this.font, Component.translatable("screen.itemglint.quick_config.summary.rules", this.ruleMode.label()), x, y, opaque(0xE0E6EE), false);
        y += 18;
        graphics.drawString(this.font, Component.translatable("screen.itemglint.quick_config.summary.hotkey"), x, y, opaque(0xAAB3C3), false);
        y += 12;
        graphics.drawString(this.font, ShaderKeyMappings.OPEN_QUICK_CONFIG.getTranslatedKeyMessage(), x, y, opaque(0xFFFFFF), false);
        y += 22;

        graphics.drawString(this.font, Component.translatable("screen.itemglint.quick_config.primary_color"), x, y, opaque(0xA7E7FF), false);
        drawSwatch(graphics, swatchX, y - 1, getColor(ColorTarget.PRIMARY), 16, 10);
        y += 16;
        graphics.drawString(this.font, Component.translatable("screen.itemglint.quick_config.secondary_color"), x, y, opaque(0xFFADD9), false);
        drawSwatch(graphics, swatchX, y - 1, getColor(ColorTarget.SECONDARY), 16, 10);
        y += 18;

        Component activeTarget = this.activeColorTarget.label();
        graphics.drawString(this.font, Component.translatable("screen.itemglint.quick_config.summary.active_target", activeTarget), x, y, opaque(0xAAB3C3), false);
        y += 12;
        graphics.drawString(this.font, Component.literal(toHex(getColor(this.activeColorTarget))), x, y, opaque(0xFFFFFF), false);
    }
    ...
    private void buildColorsPage(int left, int top) {
        ...
        this.hexField.setTextColor(opaque(0xE0E0E0));
    ...
    private <T> CycleButton<T> addEnumRow(int left, int y, Component label, T[] values, T currentValue, Consumer<T> consumer, Function<T, Component> labeler, Component tooltip) {
        ...
        int fieldWidth = getRowFieldWidth();
        addLabel(label, left, y + 6);
        CycleButton<T> widget = this.addRenderableWidget(CycleButton.<T>builder(labeler, currentValue)
                .withValues(values)
                .displayOnlyValue()
                .create(left + labelWidth, y, fieldWidth, 20, Component.empty(), (button, newValue) -> consumer.accept(newValue)));
        applyTooltip(widget, left, y, labelWidth + fieldWidth, 20, tooltip);
    ...
    private void saveAndClose() {
        ...
        ShaderClientConfig.getInstance().syncAllSettings();
        closeToParent();
    }
    ...
    private void releaseColorPickerResources() {
    ...
    private boolean dispatchMouseClickTopmost(MouseButtonEvent event, boolean doubleClick) {
    ...
    private void onHexChanged(String value) {
        ...
        try {
            ...
            this.hexField.setTextColor(opaque(0xFF8080));
        }
    }
    ...
    private static final class ColorPickerBoxWidget extends AbstractWidget {
        ...
        private ColorPickerBoxWidget(int x, int y, int width, int height, float hue, float saturation, float value, BiConsumer<Float, Float> callback) {
            ...
            this.callback = callback;
            this.textureId = Identifier.fromNamespaceAndPath("itemglint", "dynamic/color_picker_" + NEXT_TEXTURE_ID.incrementAndGet());
        ...
        protected void renderWidget(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
            ...
            if (this.texture != null) {
                graphics.blit(RenderPipelines.GUI_TEXTURED, this.textureId, this.getX(), this.getY(), 0.0F, 0.0F, this.width, this.height, this.width, this.height);
            }
        ...
        private void setColor(float hue, float saturation, float value) {
            ...
            if (differs(this.value, clampedValue)) {
        ...
        private void ensureTexture() {
            if (this.texture != null && !this.textureDirty && !differs(this.renderedValue, this.value)) {
                return;
            }
            ...
            if (this.texture == null) {
                ...
                this.texture = new DynamicTexture(() -> "itemglint_color_picker", image);
            ...
            for (int py = 0; py < this.height; py++) {
                ...
                for (int px = 0; px < this.width; px++) {
                    ...
                    image.setPixel(px, py, 0xFF000000 | Mth.hsvToRgb(sampleHue, sampleSaturation, this.value));
                }
            }
    ...
    private static final class BrightnessBarWidget extends AbstractWidget {
        ...
        private BrightnessBarWidget(int x, int y, int width, int height, float hue, float saturation, float value, Consumer<Float> callback) {
            ...
            this.callback = callback;
            this.textureId = Identifier.fromNamespaceAndPath("itemglint", "dynamic/brightness_bar_" + NEXT_TEXTURE_ID.incrementAndGet());
            setColor(hue, saturation, value);
        }
        ...
        private void setColor(float hue, float saturation, float value) {
            ...
            if (differs(this.hue, normalizedHue) || differs(this.saturation, clampedSaturation)) {
                this.textureDirty = true;
            }
        ...
        private void ensureTexture() {
            if (this.texture != null && !this.textureDirty && !differs(this.renderedHue, this.hue) && !differs(this.renderedSaturation, this.saturation)) {
                return;
            }
            ...
            if (this.texture == null) {
                image = new NativeImage(this.width, this.height, false);
                this.texture = new DynamicTexture(() -> "itemglint_brightness_bar", image);
                Minecraft.getInstance().getTextureManager().register(this.textureId, this.texture);
            } else {
                image = this.texture.getPixels();
            }
    ...
    private static boolean differs(float first, float second) {
        return Float.isNaN(first) || Float.isNaN(second) || Math.abs(first - second) > 0.0001F;
    }
}
